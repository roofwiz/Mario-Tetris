...
class AssetEditor:
        ...
        for category in sorted(self.sprite_data.keys()):
            ...
            if is_selected:
                for sprite_name in sorted(self.sprite_data[category].keys()):
                    ...
                    # Edit button
                    edit_btn = pygame.Rect(580, y_offset + 10, 60, 25)
                    btn_color = BUTTON_HOVER if self.editing_mode and is_sprite_selected else BUTTON_COLOR
                    pygame.draw.rect(screen, btn_color, edit_btn, border_radius=3)
                    edit_text = font_small.render("EDIT", True, TEXT_COLOR)
                    screen.blit(edit_text, (595, y_offset + 15))
    ...
    def draw_sprite_details(self):
        ...
        if self.selected_category and self.selected_sprite:
            ...
            # Instructions
            if self.editing_mode:
                inst = font_small.render("Arrow keys: ±1px  |  Shift+Arrows: ±5px  |  Click sheet to set position  |  ESC: Cancel", True, LABEL_COLOR)
            else:
